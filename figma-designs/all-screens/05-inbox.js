...
screen.name="01-inbox-chat - Inbox";
screen.resize(1440,900);
screen.fills=[{type:'SOLID',color:BG}];
screen.x=0;screen.y=4800;
  ...
  // Nav: Inbox
  var nav1=frame(sidebar,"Nav Inbox",240,36,{x:10,y:120,color:PRIMARY ,radius:8});
  
  txt(nav1,"Inbox",{style:"Medium",size:13,color:WHITE,x:36,y:9});
  // Nav: Contacts
...
// INBOX / Chat Ochiq
var screen=figma.createFrame();
screen.name="02-chat-open - Chat Ochiq";
screen.resize(1440,900);
screen.fills=[{type:'SOLID',color:BG}];
screen.x=1560;screen.y=4800;
...
// INBOX / Info Panel
var screen=figma.createFrame();
screen.name="03-info-sidebar - Info Panel";
screen.resize(1440,900);
screen.fills=[{type:'SOLID',color:BG}];
screen.x=3120;screen.y=4800;
...
return {success:true, section:"INBOX", screens:3};
